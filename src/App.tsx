...
import { 
  ...
  Cpu, Cloud, Network, Eye, TrendingUp, ArrowRight
} from 'lucide-react'
...
// ========== Feature 1: Interactive Pricing Tiers ==========
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${tier.popular ? 'bg-blue-100' : 'bg-slate-100'}`}>
              <tier.icon className={`w-6 h-6 ${tier.popular ? 'text-blue-600' : 'text-slate-600'}`} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl font-bold text-slate-900">{tier.name}</h3>
                {tier.popular && <Badge className="bg-blue-600 text-white text-xs">POPULAR</Badge>}
              </div>
...
function FeatureConfirmation() {
  ...
  const tiers = [
    { id: 'essential', name: 'Essential', price: '$300-500', turnaround: '48-72 hours', discount: 'Year 1: $180-300 (40% off)', target: 'Seed-stage investments', icon: FileText, services: ['AI-automated screening and data collection', 'Identity & credential verification', 'Litigation & media screening', 'Basic financial health check', 'Automated red flag detection'] },
    { id: 'professional', name: 'Professional', price: '$800-1,500', turnaround: '5-7 business days', discount: 'Year 1: $560-1,050 (40% off)', target: 'Series A investments', icon: Users, popular: true, services: ['Founder behavioral interview (60 min)', 'Reference verification (3+ contacts)', 'Basic operational assessment', 'Financial health analysis', 'Management team background check', 'Customer validation calls (2-3)', 'Network mapping & reputation check'] },
    { id: 'enterprise', name: 'Enterprise', price: '$2,500-5,000', turnaround: '10-15 business days', discount: 'Year 1: $1,750-3,500 (40% off)', target: 'Series B+ transactions', icon: Shield, services: ['Extended site presence (2-3 days)', 'In-depth network mapping & reputation', 'Forensic document analysis', 'Comprehensive management assessment', 'Supply chain verification', 'Competitive landscape analysis', 'Regulatory compliance deep-dive', 'Exit scenario evaluation'] }
  ]
  return (
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          {tiers.map((tier) => (<PricingTierCard key={tier.id} tier={tier} isExpanded={expandedTier === tier.id} onToggle={() => setExpandedTier(expandedTier === tier.id ? null : tier.id)} />))}
        </div>
        <Card>
          <CardHeader>
            <div className="flex items-start gap-4">
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-2 mb-4">
              <div className="text-center p-2 bg-slate-50 rounded-lg"><div className="text-lg font-bold text-amber-600">40-60%</div><div className="text-xs text-slate-500">Cost Reduction</div></div>
              <div className="text-center p-2 bg-slate-50 rounded-lg"><div className="text-lg font-bold text-amber-600">70%</div><div className="text-xs text-slate-500">AI Processing</div></div>
              <div className="text-center p-2 bg-slate-50 rounded-lg"><div className="text-lg font-bold text-amber-600">30%</div><div className="text-xs text-slate-500">Human Review</div></div>
            </div>
            <div className="space-y-2">
              {[{ step: 1, title: 'AI Data Collection', time: '2-4 hours' }, { step: 2, title: 'AI Analysis', time: '1-2 hours' }, { step: 3, title: 'Human Verification', time: '2-5 days' }, { step: 4, title: 'Quality Review', time: '1-2 days' }].map((s) => (
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center"><Globe className="w-6 h-6 text-green-600" /></div>
              <div>
          ...
          <CardContent>
            <div className="space-y-2">
              {[{ country: 'Vietnam', phase: 'Phase 1', deals: '141 deals', funding: '$2.3B' }, { country: 'Philippines', phase: 'Phase 2', deals: '80-100 deals', funding: '$1B+' }, { country: 'Indonesia', phase: 'Phase 3', deals: 'Largest market', funding: 'Post-eFishery' }, { country: 'Malaysia', phase: 'Phase 3', deals: 'RM 17.58B', funding: 'PE/VC hub' }].map((m) => (
                <div key={m.country} className="p-3 rounded-lg border border-slate-200">
                  <div className="flex items-center justify-between mb-1"><span className="font-semibold text-slate-900">{m.country}</span><Badge className="bg-slate-100 text-slate-600 text-xs">{m.phase}</Badge></div>
...
// ========== Feature 4: Interactive Risk Assessment ==========
function RiskAssessment() {
  const [activeCategory, setActiveCategory] = useState('financial')
  const riskCategories: any = {
    financial: { name: 'Financial Risk', weight: '25%', description: 'Assessment of financial health, revenue validation, and fiscal stability', indicators: ['Revenue verification & validation', 'Cash flow analysis', 'Debt & liability assessment', 'Financial statement review', 'Burn rate evaluation'], sampleFinding: { severity: 'high', title: 'Revenue Concentration Risk', description: '65% of revenue from single customer creates significant dependency', score: 7.2, impact: 'High customer churn risk if contract is lost' } },
    operational: { name: 'Operational Risk', weight: '25%', description: 'Evaluation of business operations, supply chain, and execution capability', indicators: ['Site verification & operational audit', 'Customer validation calls', 'Supplier relationship assessment', 'Process documentation review', 'Key personnel dependency'], sampleFinding: { severity: 'medium', title: 'Limited Operational History', description: 'Company operating for only 18 months with limited track record', score: 5.4, impact: 'Unproven scalability and operational resilience' } },
    legal: { name: 'Legal/Compliance', weight: '20%', description: 'Review of legal standing, regulatory compliance, and IP protection', indicators: ['Litigation & dispute check', 'Regulatory compliance verification', 'IP ownership & protection', 'Contract review', 'Corporate governance assessment'], sampleFinding: { severity: 'low', title: 'Standard IP Assignments', description: 'All IP properly assigned to company with clean ownership chain', score: 2.1, impact: 'No legal risks identified in IP portfolio' } },
    founder: { name: 'Founder/Team', weight: '20%', description: 'Background check on founders, management team assessment, and references', indicators: ['Founder background verification', 'Reference interviews (3+)', 'Track record analysis', 'Management team assessment', 'Character & reputation check'], sampleFinding: { severity: 'high', title: 'Prior Business Failure', description: 'Founder previously led company that filed for bankruptcy in 2019', score: 8.5, impact: 'Potential governance and decision-making concerns' } },
    market: { name: 'Market Risk', weight: '10%', description: 'Analysis of market size, competitive position, and growth trajectory', indicators: ['Market size validation (TAM/SAM/SOM)', 'Competitive positioning', 'Growth trajectory analysis', 'Industry trend assessment', 'Customer acquisition metrics'], sampleFinding: { severity: 'medium', title: 'Competitive Market Entry', description: 'Entering saturated market with 15+ established competitors', score: 5.8, impact: 'Differentiation and customer acquisition challenges expected' } }
  }
  const currentCategory = riskCategories[activeCategory]
  const getSeverityColor = (severity: string) => ({ low: { bg: 'bg-green-100', border: 'border-green-300', text: 'text-green-800', badge: 'bg-green-500' }, medium: { bg: 'bg-amber-100', border: 'border-amber-300', text: 'text-amber-800', badge: 'bg-amber-500' }, high: { bg: 'bg-red-100', border: 'border-red-300', text: 'text-red-800', badge: 'bg-red-500' } }[severity] || { bg: 'bg-amber-100', border: 'border-amber-300', text: 'text-amber-800', badge: 'bg-amber-500' })
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-slate-600 max-w-2xl mx-auto">5-category risk scoring system with detailed indicators and sample findings.</p>
      ...
      <div className="flex flex-wrap gap-2 bg-slate-100 p-2 rounded-xl">
        {Object.entries(riskCategories).map(([key, category]: [string, any]) => (
          <button key={key} onClick={() => setActiveCategory(key)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeCategory === key ? 'bg-white text-slate-900 shadow-sm ring-2 ring-blue-500' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200'}`}>
            {category.name}
      ...
      <Card className="border-t-4 border-t-blue-500">
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <Badge className="bg-blue-100 text-blue-700 text-sm px-3 py-1">Weight: {currentCategory.weight}</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid lg:grid-cols-2 gap-6">
            <div>
              <h4 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-blue-500" />Risk Indicators</h4>
              <ul className="space-y-2">
                {currentCategory.indicators.map((indicator: string, idx: number) => (
                  <li key={idx} className="flex items-start gap-2 text-sm text-slate-600 p-2 bg-slate-50 rounded-lg"><span className="text-blue-500 mt-0.5">•</span>{indicator}</li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-slate-900 mb-3 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-amber-500" />Sample Finding</h4>
              <div className={`p-4 rounded-xl border-2 ${colors.bg} ${colors.border}`}>
                <div className="flex items-center gap-2 mb-2"><span className={`w-3 h-3 rounded-full ${colors.badge}`}></span><span className={`text-xs font-bold uppercase ${colors.text}`}>{currentCategory.sampleFinding.severity} Risk</span></div>
                <h5 className={`font-bold text-lg mb-1 ${colors.text}`}>{currentCategory.sampleFinding.title}</h5>
                <p className={`text-sm mb-3 ${colors.text}`}>{currentCategory.sampleFinding.description}</p>
                <div className="flex items-center justify-between pt-3 border-t border-current border-opacity-20">
                  <div><span className={`text-xs ${colors.text} opacity-70`}>Risk Score</span><p className={`font-bold text-xl ${colors.text}`}>{currentCategory.sampleFinding.score}/10</p></div>
                  <div className="text-right"><span className={`text-xs ${colors.text} opacity-70`}>Impact</span><p className={`text-sm ${colors.text}`}>{currentCategory.sampleFinding.impact}</p></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <Card className="bg-slate-50">
        <CardHeader><h3 className="font-semibold text-slate-900">Overall Risk Score Breakdown</h3></CardHeader>
        <CardContent>
          <div className="space-y-3">
            {Object.entries(riskCategories).map(([key, category]: [string, any]) => {
              ...
              return (
                <div key={key} onClick={() => setActiveCategory(key)} className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-all ${isActive ? 'bg-white shadow-sm ring-2 ring-blue-200' : 'hover:bg-white'}`}>
                  <div className="w-24 text-sm font-medium text-slate-700">{category.name}</div>
                  <div className="flex-1"><div className="h-2 bg-slate-200 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${score <= 3 ? 'bg-green-500' : score <= 6 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${(score / 10) * 100}%` }} /></div></div>
                  <div className="w-16 text-right"><span className={`font-bold ${score <= 3 ? 'text-green-600' : score <= 6 ? 'text-amber-600' : 'text-red-600'}`}>{score}</span><span className="text-xs text-slate-400 ml-1">{category.weight}</span></div>
                </div>
              )
            })}
          </div>
          <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between"><span className="text-sm text-slate-600">Overall Weighted Score</span><span className="text-2xl font-bold text-amber-600">5.8/10</span></div>
        </CardContent>
      </Card>
    </div>
  )
...
function FlowDiagram() {
  ...
  return (
    <div className="space-y-8">
          ...
          <Card key={step.id} className="relative">
            <CardHeader>
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${step.color === 'blue' ? 'bg-blue-100' : step.color === 'amber' ? 'bg-amber-100' : 'bg-green-100'}`}>
                  <step.icon className={`w-5 h-5 ${step.color === 'blue' ? 'text-blue-600' : step.color === 'amber' ? 'text-amber-600' : 'text-green-600'}`} />
                </div>
                <div>
      ...
      <Card className="bg-slate-50">
        <CardHeader><h3 className="font-semibold text-slate-900">Visual Process Flow</h3></CardHeader>
        <CardContent>
...
function SystemArchitecture() {
  ...
  const layers = [
    { id: 'presentation', name: 'Presentation Layer', icon: Eye, color: 'blue', components: [{ name: 'Web Dashboard', tech: 'React + TypeScript' }, { name: 'Mobile App', tech: 'React Native' }, { name: 'API Gateway', tech: 'Kong/AWS' }, { name: 'Admin Panel', tech: 'React + shadcn/ui' }] },
    { id: 'application', name: 'Application Layer', icon: Cpu, color: 'amber', components: [{ name: 'Orchestration Service', tech: 'Node.js / Python' }, { name: 'AI/ML Engine', tech: 'Python + TensorFlow' }, { name: 'Report Generator', tech: 'Node.js + Puppeteer' }, { name: 'Document Parser', tech: 'Python + Tesseract' }] },
    { id: 'data', name: 'Data Layer', icon: Database, color: 'green', components: [{ name: 'Primary Database', tech: 'PostgreSQL' }, { name: 'Document Store', tech: 'MongoDB' }, { name: 'Cache Layer', tech: 'Redis' }, { name: 'Search Engine', tech: 'Elasticsearch' }] },
    { id: 'integration', name: 'Integration Layer', icon: Network, color: 'purple', components: [{ name: 'Registry Connectors', tech: 'REST APIs' }, { name: 'Credit Bureau APIs', tech: 'SOAP/REST' }, { name: 'News Aggregator', tech: 'Python + Scrapy' }, { name: 'Webhook Handler', tech: 'Node.js' }] },
    { id: 'infrastructure', name: 'Infrastructure Layer', icon: Cloud, color: 'slate', components: [{ name: 'Container Orchestration', tech: 'Kubernetes (EKS)' }, { name: 'CDN', tech: 'CloudFront' }, { name: 'Load Balancer', tech: 'ALB/NLB' }, { name: 'Monitoring', tech: 'Prometheus + Grafana' }] }
  ]
  return (
    <div className="space-y-8">
      <div className="text-center">
...
function DemoDashboard() {
  ...
  const getStatusColor = (status: string) => ({ completed: 'bg-green-100 text-green-700', in_progress: 'bg-blue-100 text-blue-700', pending: 'bg-slate-100 text-slate-700' }[status] || 'bg-slate-100 text-slate-700')
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'markets' && (
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[{ country: 'Vietnam', deals: 141, funding: '$2.3B', active: 12 }, { country: 'Philippines', deals: 85, funding: '$1.1B', active: 8 }, { country: 'Indonesia', deals: 156, funding: '$3.2B', active: 15 }, { country: 'Malaysia', deals: 67, funding: '$890M', active: 6 }].map((m) => (
            <Card key={m.country}>
              <CardHeader><h3 className="font-semibold text-slate-900">{m.country}</h3></CardHeader>
              <CardContent>
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Button variant="primary" className="bg-blue-600 text-white"><Play className="w-4 h-4" /> Start Demo</Button>
          </div>
        </div>
      </header>
